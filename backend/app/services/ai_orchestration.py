...
class AIOrchestrationService:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.info("Gemini SDK version: %s", sdk_version)
            print("[AIOrchestrationService] Using generative model:", getattr(self, "chosen_model", "unknown"))
            print("[AIOrchestrationService] Gemini SDK version:", sdk_version)
        except Exception:
            pass
        self.logger = logging.getLogger("app.ai_orchestration")
    ...
    def generate_interviewer_turn(
        ...
        # Debug: log the full prompt, candidate answer, and context for one-off troubleshooting
        try:
            self.logger.debug("Interviewer turn prompt:\n%s", prompt)
            print("[AIOrchestrationService] Interviewer turn prompt:\n", prompt)
            print("[AIOrchestrationService] Interviewer turn metadata:")
            print("  question:", question)
            print("  answer:", answer)
            print("  category:", category)
            print("  difficulty:", difficulty)
            print("  context:", json.dumps(context or {}, indent=2, default=str))
        except Exception:
            pass
        ...
        try:
            ...
            raise RuntimeError(f"Gemini response could not be normalized into a valid recruiter turn. Raw response:\n{response_text}")

        except Exception as e:
            try:
                self.logger.exception("Interviewer turn failed: %s", e)
                print(f"[AIOrchestrationService] Interviewer turn failed: {type(e).__name__}: {e}")
            except Exception:
                pass
            raise
    ...
    def _postprocess_recruiter_message(self, message: str, answer: Optional[str] = None, category: Optional[str] = None) -> str:
        """Improve realism: avoid repeated canned phrases, add contextual follow-ups and vary tone."""
        if not message:
            return message
        msg = message.strip()

        # Common patterns to avoid or replace
        banned_patterns = [r"\btell me more\b", r"\byou said\b", r"\bcan you elaborate\b", r"\bcould you elaborate\b", r"\bplease elaborate\b"]
        if any(re.search(p, msg, flags=re.I) for p in banned_patterns):
            quote = self._get_candidate_quote(answer or msg)
            key = (category or "").lower()
            # Contextual templates for each interview type
            templates_with_quote = {
                "technical": [
                    "I noticed you mentioned '{q}' — could you walk me through how that worked?",
                    "When you said '{q}', what key decision did you make there?",
                ],
                "behavioral": [
                    "You mentioned '{q}'. What happened next and what did you learn?",
                    "That example is interesting — how did the team react to '{q}'?",
                ],
                "communication": [
                    "When you said '{q}', who was the audience and how did you tailor the message?",
                    "Can you tell me how you'd explain '{q}' to someone non-technical?",
                ],
                "hr": [
                    "You mentioned '{q}' — which part of that experience mattered most to you?",
                    "What about '{q}' made you take that approach?",
                ],
            }
            templates_no_quote = {
                "technical": [
                    "Can you expand slightly on the steps you'd take to implement that?",
                    "How would you handle any performance or scaling issues there?",
                ],
                "behavioral": [
                    "Could you tell me what you learned from that experience?",
                    "What would you do differently next time?",
                ],
                "communication": [
                    "How would you summarise that in a sentence for a non-technical stakeholder?",
                    "What's the core point you want people to take away?",
                ],
                "hr": [
                    "Which part of that experience do you think is most relevant to this role?",
                    "What motivated you to make that choice?",
                ],
            }

            if quote:
                choices = [t.format(q=quote) for t in templates_with_quote.get(key, [])]
                if not choices:
                    choices = [f"You mentioned '{quote}' — could you tell me more about that?"]
            else:
                choices = templates_no_quote.get(key, [])
                if not choices:
                    choices = ["Could you expand a bit on that?"]

            # Pick a phrase avoiding recent repetition
            pick = None
            for _ in range(4):
                candidate = random.choice(choices)
                if candidate not in self._recent_phrases:
                    pick = candidate
                    break
            if not pick:
            ...
            try:
                self._recent_phrases.append(pick)
        ...
        # Shorten long sentences to stay conversational
        ...
        try:
            if msg not in self._recent_phrases:
                self._recent_phrases.append(msg)
                if len(self._recent_phrases) > self.RECENT_PHRASES_LIMIT:
                    self._recent_phrases = self._recent_phrases[-self.RECENT_PHRASES_LIMIT :]
        except Exception:
            pass
    ...
    def _fallback_interviewer_turn(
        ...
        recruiter_msg = self._fallback_recruiter_feedback(question, answer, category)
        # Post-process to avoid canned phrases and add variation
        recruiter_msg = self._postprocess_recruiter_message(recruiter_msg, answer=answer, category=category)
        base["recruiter_feedback"] = recruiter_msg
        return base

    def _detect_answer_context(self, answer: str) -> Dict[str, bool]:
        lower = (answer or "").lower()
        return {
            "frontend": any(term in lower for term in [
                "frontend", "ui", "ux", "react", "vue", "angular", "html", "css", "javascript", "typescript",
                "responsive", "design", "accessibility", "user interface", "user experience"
            ]),
            "backend": any(term in lower for term in [
                "backend", "server", "api", "microservice", "kubernetes", "docker", "node", "python", "java", "go",
                "spring", "flask", "express", "service", "endpoint"
            ]),
            "database": any(term in lower for term in [
                "database", "sql", "nosql", "postgres", "mysql", "mongo", "redis", "dynamodb", "cassandra", "sql server", "oracle",
                "query", "index", "transaction", "schema", "relational"
            ]),
            "teamwork": any(term in lower for term in [
                "team", "collaborat", "collaboration", "stakeholder", "peer", "partner", "paired", "lead", "managed", "manager"
            ]),
            "conflict": any(term in lower for term in [
                "conflict", "disagree", "push back", "misunderstand", "tension", "issue", "heated", "difficult conversation"]),
            "project": any(term in lower for term in [
                "project", "launch", "release", "build", "developed", "ship", "feature", "product", "initiative", "roadmap"]),
            "coding": any(term in lower for term in [
                "code", "coding", "program", "programming", "algorithm", "implementation", "debug", "refactor"]),
            "language": any(term in lower for term in [
                "python", "java", "javascript", "typescript", "c#", "golang", "go", "ruby", "c++", "rust", "php"]),
            "nervousness": any(term in lower for term in [
                "nervous", "anxious", "uncertain", "unsure", "hesitant", "maybe", "perhaps"]),
            "confidence": any(term in lower for term in [
                "confident", "sure", "certain", "clear", "definitely", "absolutely"]),
        }
    ...
    def _generate_contextual_follow_up(
        ...
        flags = self._detect_answer_context(answer)

        candidates: List[str] = []

        if flags["frontend"]:
            candidates.extend([
                "What got you interested in frontend development specifically?",
                "Do you enjoy the design side more or the technical implementation side?",
                "Was that work more about user experience or about component architecture?",
                "Which part of the UI did you find most challenging to implement?",
            ])
            if quote:
                candidates.append(f"You mentioned '{quote}'; was that part of a project or a team build?")

        if flags["database"] or flags["backend"]:
            candidates.extend([
                "That’s a practical approach. Have you worked with any real-world database system yet?",
                "How would you choose between SQL and NoSQL for that use case?",
                "What backend trade-off matters most when you build a system like that?",
                "Which database decision had the biggest impact on your design?",
            ])
            if quote and "database" in answer_lower:
                candidates.append(f"When you said '{quote}', which database or backend choice were you referencing?")

        if flags["teamwork"] or flags["conflict"]:
            candidates.extend([
                "That’s a mature way to handle that. Was that during a project or a team assignment?",
                "What role did you play on the team and how did you influence the outcome?",
                "How did you coordinate with others to make that work?",
            ])
            if flags["conflict"]:
                candidates.append("How did you resolve that disagreement while keeping the project on track?")

        if flags["project"]:
            candidates.extend([
                "Was that something you built as part of a project or on your own?",
                "Which part of that experience was the most meaningful for you?",
            ])

        if flags["coding"] or flags["language"]:
            candidates.extend([
                "If you were coding that today, which language or framework would you choose and why?",
                "Have you used that language in production, or was it more of a prototype?",
            ])

        if flags["nervousness"]:
            candidates.extend([
                "It sounds like you were being careful there — what part of that answer do you feel most confident about?",
                "What was the one detail you were sure of in that explanation?",
            ])

        if flags["confidence"]:
            candidates.extend([
                "That sounds confident — what part of that project or technical choice do you feel strongest about?",
                "Which aspect of that answer would you point to as your clearest example?",
            ])
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                try:
                    ...
                    self.logger.debug("Model response text: %s", getattr(response, 'text', None))
                    self.logger.debug("Model response candidates: %s", getattr(response, 'candidates', None))
                except Exception:
                    pass
                ...
                try:
                    print("[AIOrchestrationService] Gemini raw response object:", repr(response))
                    print("[AIOrchestrationService] Gemini response text:", text)
                    print("[AIOrchestrationService] Gemini response candidates:", getattr(response, 'candidates', None))
                except Exception:
                    pass
                return text or ""
            except Exception as e:
...
Be fair, specific, and hiring-focused."""
    ...
    def _fallback_recruiter_feedback(self, question: str, answer: str, category: str) -> str:
        ...
        # Use context flags for more specific recruiter reactions
        if flags["frontend"] and category.lower() == "technical":
            return "That sounds UI-focused — what frontend pattern or UI choice did you enjoy most there?"
        if flags["database"] and category.lower() == "technical":
            return "Makes sense — which database design decision was most important for that implementation?"
        if flags["teamwork"] and category.lower() in {"behavioral", "hr"}:
            return "Nice — what role did you play on that team, and how did you coordinate with others?"
        if flags["project"]:
            return "Was that part of a project or something you built independently?"
